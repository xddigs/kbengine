package org.kbeng.rpg.graphics;

import org.kbeng.engine.graphics.*;

import org.kbeng.rpg.entity.Enemy;
import org.kbeng.rpg.entity.enemies.Goblin;
...
public class ResourceManager {
    ...
    private static Map<BlockShape, Mesh> createShapedSelectionMeshes() {
        ...
        for (BlockShape shape : BlockShape.values()) {
            if (!shape.isFullCube()) meshes.put(shape, Mesh.selection(toOutlineBoxes(shape)));
        }
        return meshes;
    }

    private static float[][] toOutlineBoxes(BlockShape shape) {
        BlockShape.Box[] source = shape.getBoxes();
        float[][] boxes = new float[source.length][6];
        for (int i = 0; i < source.length; i++) {
            BlockShape.Box box = source[i];
            boxes[i] = new float[]{box.minX(), box.minY(), box.minZ(),
     ...
     * @return the {@link SpriteSheet} representing the book (static) sheet
     */
    public SpriteSheet getBookUI() {
        return bookUI;
    }

    /**
     * Returns the book animation sheet.
     * @return the {@link SpriteSheet} representing the book sheet
     */

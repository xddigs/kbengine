package org.kbeng.rpg.entity.plyr;

import org.kbeng.rpg.data.Gamemode;
import org.kbeng.rpg.data.Cause;
import org.kbeng.rpg.data.Difficulty;
import org.kbeng.rpg.data.InventorySlot;
import org.kbeng.rpg.data.Reputation;
import org.kbeng.rpg.data.Seed;
import org.kbeng.rpg.data.SoundGroup;
import org.kbeng.rpg.data.StartingKit;
import org.kbeng.rpg.entity.Player;
import org.kbeng.rpg.entity.WorldItem;
...
public final class PlayerGameplay {
    ...
    public void initialize() {
        ...
        float maxDefense = MAX_HITPOINTS + (Player.plyr.getLevel() * 2f);
        ...
        player.setMaxHitpoints(MAX_HITPOINTS);
        player.setHitpoints(MAX_HITPOINTS);
        player.setMaxHunger(MAX_HUNGER);
        player.setHunger(MAX_HUNGER);
        player.setSpeed(SPEED);
        player.setReputation(Reputation.NEUTRAL);
    ...
    public void respawn() {
        ...
        player.setReputation(Reputation.NEUTRAL);
        player.setGamemode(Gamemode.SURVIVAL);
        player.setIsOffGroundTimer(0.0f);
    ...
    public void add(Item item, int amount) {
        ...
        log.trace("Added x{} of {} to inventory", amount, item.getName());
    }
    ...
    public void add(Item item) {
        add(item, 1);
        log.trace("Added x1 of {} to inventory", item.getName());
    }
    ...
    public void addToBackpack(Item item, int amount) {
        if (player.getBackpack().hasBackpackEquipped()) player.getBackpack().add(item, amount);
        log.trace("Added x{} of {} to backpack", amount, item.getName());
    }
    ...
    public void addToBackpack(Item item) {
        ...
        log.trace("Added x1 of {} to backpack", item.getName());
    }
    ...
    public void removeFromBackpack(Item item, int amount) {
        if (player.getBackpack().hasBackpackEquipped()) player.getBackpack().remove(item, amount);
        log.trace("Removed x{} of {} from backpack", amount, item.getName());
    }
    ...
    public void removeFromBackpack(Item item) {
        ...
        log.trace("Removed x1 of {} from backpack", item.getName());
    }
    ...
    public void remove(Item item, int amount) {
        if (!player.getGamemode().isGodmode()) {
            ...
            log.trace("Removed x{} of {} to inventory", amount, item.getName());
        }
    }
    ...
    public void remove(Item item) {
        if (!player.getGamemode().isGodmode()) {
            ...
            log.trace("Removed x1 of {} from inventory", item.getName());
        }
    }
    ...
    public void clear() {
        for (Item item : List.copyOf(player.getInventory().getItems().keySet())) {
            if (item != null && !(item instanceof Backpack)) remove(item);
        }
    ...
    public Item get(int index) {
        return player.getInventory().get(index);
    }
    ...
    public Item get(Item item) {
        return player.getInventory().get(item);
    }
    ...
    public int getAmount(Item item) {
        return player.getInventory().getAmount(item);
    }
    ...
    public boolean hasSpace() {
        return !player.getInventory().isFull() || player.getBackpack().hasBackpackEquipped() && !player.getBackpack().isFull();
    }
    ...
    public boolean hasSeeds() {
        return player.getInventory().hasItemOfType(Seed.class);
    }
